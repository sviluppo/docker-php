...
RUN buildDeps=" \
    ...
    && phive install --global --trust-gpg-keys C00543248C87FB13 composer-normalize \
...
RUN curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer \
    ...
    #&& composer global require composer-unused/composer-unused-plugin
ENV COMPOSER_ALLOW_SUPERUSER=1

COPY "8.1.php.ini" "/usr/local/etc/php/php.ini"
